...
class MainActivity : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        //查詢機場代碼
        val item = arrayOf("TPE 桃園", "TSA 松山", "CYI 嘉義", "CMJ 七美", "GNI 綠島","HUN 花蓮",
                ...
                override fun onResponse(call: okhttp3.Call, response: Response) {
                    if (response.isSuccessful) {
                            ...
                            for (i in 0 until jsonObject.length()) {
                                //更新時間
                                var updateTime = jsonObject.getJSONObject(i).getString("UpdateTime")
                                ...
                                //預計起飛時間
                                var departureTime =
                                    jsonObject.getJSONObject(i).getString("ScheduleDepartureTime")
    ...
    //                        println("預計起飛時間:${departureTime}")
                                ...
                                if (updateTime > departureTime) {
                                    if (jsonObject.getJSONObject(i).has("ActualDepartureTime")) {
                                        //實際起飛時間
                                        if (count >= 10) {
                                            break
                                        }
                                        var actdeparture =
                                            jsonObject.getJSONObject(i).getString("ActualDepartureTime")
                                        count += 1
                                        index = i
    ...
    //                            println("實際起飛時間:${actTime}")
                                        ...
                                        //航班
                                        val airlineID =
                                            jsonObject.getJSONObject(index).getString("AirlineID")
                                        val flightNumber =
                                            jsonObject.getJSONObject(index).getString("FlightNumber")
    ...
    //                            println("目的地:${destination}")
                                        ...
                                        //航廈
                                        val terminal =
                                            jsonObject.getJSONObject(index).getString("Terminal")
    //                            println("航廈:${terminal}")

                                        //登機門
                                        val gate = jsonObject.getJSONObject(index).getString("Gate")
    //                            println("登機門:${gate}")
